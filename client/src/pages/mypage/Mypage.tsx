...
import baseProfile from '../../assets/images/profile.png';
...
function Mypage() {
  ...
  return (
    <div style={{ marginTop: '160px', display: 'flex', justifyContent: 'center' }}>
      <MyPageWrapper>
        <WelcomeText>
          안녕하세요, <span style={{ color: 'var(--purple)' }}>{nickname}</span>님!
        </WelcomeText>
        <section
          style={{ borderTop: '2px solid var(--light-purple)', margin: '20px', padding: '30px' }}
        >
          <MyInfoSection>
            <img src={baseProfile} style={{ width: '200px', paddingRight: '10px' }} />
            <MyInfoDetail>
              {editMode ? (
                <EditableNickname
                  nickname={nickname}
        ...
        <MyOrderSection>
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            <h2 onClick={resetPage} style={{ cursor: 'pointer' }}>
              나의 주문
            </h2>
          ...
          <MyOrderLists>
            ...
            {filteredOrderlist.orderInfos.length === 0 ? (
              <>
                ...
                />
                <p style={{ color: 'var(--dark-gray)', fontWeight: '800', marginBottom: '10px' }}>
                  주문내역이 없습니다.
                </p>
              </>
            ) : (
